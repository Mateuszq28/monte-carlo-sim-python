...
class ProjectionResultRecordsDF():
    ...
    def throw(self, resultRecordsDF: pd.DataFrame, input_shape, axis, xray):
        """
        The function will take first value from object3D.body (recovered from resultRecordsDF), which is different then values in self.omit, seen perpendicularly to axis.
    ...
    def set_z_as_flat_axis(self, resultRecordsDF, flataxis):
        ...
        print("resultRecordsDF", resultRecordsDF)
        output = resultRecordsDF.rename(columns={ax_lvl_names[0]: "x_idx", ax_lvl_names[1]: "y_idx", flat_axis_name: "z_idx"})
        print("output", output)
        return output